...
export function createGameState(config = {}) {
  ...
  return {
    ...
    //   group: PLAYER_GROUP value — for hitters: BENCH_HITTERS, PRACTICE_SQUAD, or IL.
    //          STARTING_HITTERS is kept in constants for backward compat but is no longer
    //          assigned to user or CPU hitters — use lineupSlots to identify starters.
...
export function createUserTeamObject(config = {}) {
  return {
    ...
    // Fixed 9-slot batting lineup — always C/1B/2B/3B/SS/OF/OF/OF/DH order.
    // Each entry: { slot: String, playerId: String|null }
    // playerId null = vacant slot. Source of truth for who is "starting".
    // player.group is NOT used to identify starters — use lineupSlots instead.
    lineupSlots: [],
    ...
    wins:   0,
    losses: 0,
    ties:   0,   // spring-training ties (regular season never ties)
...
function _fillDefaults(state) {
  ...
  // ── lineupSlots migration ─────────────────────────────────────────────────
  // If any team is missing lineupSlots (old save), build from STARTING_HITTERS
  // players and change their group to BENCH_HITTERS. Safe to run every load.
  const _migrateSlots = (team) => {
    if (!team || (team.lineupSlots && team.lineupSlots.length === 9)) return;
    const SLOTS   = ['C','1B','2B','3B','SS','OF','OF','OF','DH'];
    const players = state.players || {};
    const starters = (team.rosterIds || [])
      .map(id => players[id])
      .filter(p => p && p.group === 'sh'); // STARTING_HITTERS = 'sh'
    const slots = SLOTS.map(slot => ({ slot, playerId: null }));
    const used  = new Set();
    ...
    for (const id of used) {
      if (players[id]) players[id].group = 'bh'; // BENCH_HITTERS = 'bh'
    }
  };
  if (state.userTeam) _migrateSlots(state.userTeam);
  for (const team of state.leagueTeams || []) _migrateSlots(team);
