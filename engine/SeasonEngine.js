...
// Spring training: starts mid-March, 10 games over ~2.5 weeks with off days.
// Regular season: always April 1 (rolled to Monday if needed).
// No gap constant needed — the display calendar naturally produces ~1 travel day.
const SPRING_START_MONTH = 2;  // March (0-indexed = 2)
const SPRING_START_DAY   = 15; // March 15 anchor, rolled to nearest Monday (~Mar 16-18)
const SPRING_GAME_COUNT  = 10; // 10 spring training games

// Override the imported constant for spring training
// (SPRING_TRAINING_GAME_COUNT from constants.js = 20, we use our local override)
const _SPRING_GAME_COUNT = SPRING_GAME_COUNT;
...
function _generateSpringGames(realStartDate, displayStartDate, leagueTeams, year) {
  ...
  while (gameNum < _SPRING_GAME_COUNT) {
    // Skip off days
...
function _getSeasonStartDate(year) {
  // Regular season starts April 1, rolled forward to the nearest Monday.
