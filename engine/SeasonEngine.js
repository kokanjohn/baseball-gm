...
const SPRING_START_MONTH = 1;  // February (0-indexed) — runs ~Feb 15 to Mar 7
const SPRING_START_DAY   = 15; // Feb 15 anchor, rolls to nearest Monday
...
function _generateSpringGames(realStartDate, displayStartDate, leagueTeams, year) {
  ...
  let realDate      = new Date(realStartDate);    // advances through today+
  let displayDate   = new Date(displayStartDate); // advances through Mar 22+
  let gameNum       = 0;
  ...
  while (gameNum < SPRING_TRAINING_GAME_COUNT) {
    ...
    // gameTime uses the real-world date so the game fires at the correct clock time.
    // game.date uses the fictional baseball calendar date for display.
    ...
    games.push(_makeGameObject({
      ...
      opponent,
      isHome,
      date:     _isoDate(displayDate),   // fictional baseball date shown to user
      gameTime,                          // real-world Unix ms for clock comparison
      isSpring: true,
    ...
    // Skip Sunday every 3rd week (travel day simulation) — advance both dates
    if (realDate.getDay() === 0 && gameNum % 6 === 0) {
      realDate    = _addDays(realDate, 1);
      displayDate = _addDays(displayDate, 1);
    }
  }
