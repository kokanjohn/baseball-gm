...
// Spring training always starts on a Monday in late February/early March
// Season start: always the first Monday of April (real-world anchor)
const SPRING_START_MONTH = 2;  // March (0-indexed)
const SPRING_START_DAY   = 22; // ~March 22 anchor
...
export function generateUserSchedule(seasonNum, leagueTeams, startDateOverride) {
  ...
  // If a start date override is provided, anchor spring training to that date
  // (rolling forward to the next Monday). Otherwise use the canonical calendar anchor.
  let springStart;
  if (startDateOverride) {
    // Use the override date directly — this is "tomorrow" passed from startNewGame,
    // ensuring the first game is always the next calendar day at 1:05 PM.
    // No day-of-week adjustment — spring training games run daily regardless.
    springStart = new Date(startDateOverride);
  } else {
    springStart = _getSpringStartDate(year);
  }

  // Regular season always starts a fixed number of days after spring training ends
  // (spring training = 20 games × 1 game/day ≈ 20 days + travel buffer = 24 days)
  const SPRING_TO_SEASON_GAP_DAYS = 24;
  const seasonStart = startDateOverride
    ? _addDays(springStart, SPRING_TO_SEASON_GAP_DAYS)
    : _getSeasonStartDate(year);

  const schedule = [];

  // ── Spring training (20 games) ─────────────────────────────
  const springGames = _generateSpringGames(springStart, leagueTeams, year);
  schedule.push(...springGames);

  // ── Regular season (132 games) ────────────────────────────
  const regularGames = _generateRegularSeasonGames(seasonStart, leagueTeams, year);
  schedule.push(...regularGames);

  // Assign global index across the full schedule
...
function _generateSpringGames(startDate, leagueTeams, year) {
  const games  = [];
  const teams  = leagueTeams.map(t => t.name);
  let date     = new Date(startDate);
  let gameNum  = 0;

  while (gameNum < SPRING_TRAINING_GAME_COUNT) {
    const dow      = DOW_NAMES[date.getDay()];
    const isHome   = Math.random() < 0.50;
    const opponent = teams[gameNum % teams.length];

    // First spring game is always 1:05 PM (Opening Day of spring).
    // All other spring games use the same DOW-based time pool as regular season
    // so the schedule shows realistic variation (afternoon/evening) throughout.
    ...
    const gameTime = _parseGameTime(timeStr, date);
    ...
    // Skip Sunday every 3rd week (travel day simulation)
    if (date.getDay() === 0 && gameNum % 6 === 0) {
      date = _addDays(date, 1);
    }
  }
...
function _generateRegularSeasonGames(startDate, leagueTeams, year) {
  const games   = [];
  const teams   = leagueTeams.map(t => t.name);
  let date      = new Date(startDate);
  let gameIndex = 0; // 0-based within regular season

  // Build series blocks — each series is 2–4 games against one opponent
  ...
  for (const series of seriesBlocks) {
    ...
    for (let i = 0; i < seriesLen; i++) {
      ...
      // Insert All-Star Break gap after game 66
      const isPostBreak = gameIndex >= ALL_STAR_BREAK_AFTER_GAME;

      // Flag deadline window
      ...
      // Travel day between series (except consecutive home/away with same team)
      if (i === seriesLen - 1 && gameIndex < REGULAR_SEASON_GAME_COUNT) {
        ...
        if (nextSeries && nextSeries.isHome !== series.isHome) {
          date = _addDays(date, 1); // travel day
        }
      }
    }
    ...
    if (gameIndex === ALL_STAR_BREAK_AFTER_GAME) {
      date = _addDays(date, 4); // 4-day break
    }
  }
...
export function generateCPUSchedules(seasonNum, leagueTeams) {
  ...
  // Generate round-robin matchups — each pair plays ~14 times (132 / 9 ≈ 14.7)
...
function _getSpringStartDate(year) {
  // Spring training starts ~March 22 on a Monday
  const date = new Date(year, SPRING_START_MONTH, SPRING_START_DAY);
  // Roll forward to nearest Monday
  while (date.getDay() !== 1) date.setDate(date.getDate() + 1);
  return date;
}

function _getSeasonStartDate(year) {
  // Regular season starts first Monday of April
  const date = new Date(year, 3, 1); // April 1
  while (date.getDay() !== 1) date.setDate(date.getDate() + 1);
  return date;
}
