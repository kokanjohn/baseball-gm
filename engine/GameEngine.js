...
export async function startNewGame(config) {
  ...
  // Generate schedules
  const schedule        = generateUserSchedule(1, league.leagueTeams);
...
export function tick() {
  ...
  if (game.status === GAME_STATUS.SCHEDULED || game.status === GAME_STATUS.PRE_GAME_WATCH) {
    ...
    if (game.gameTime && now >= game.gameTime) {
      _activateGame(game);
      // Fall through — plays now exist, first tick will reveal play[0]
    } else {
      return null;
    }
  }
...
export async function commitGame(gameIndex) {
  ...
  if (![GAME_STATUS.FINAL, GAME_STATUS.POSTPONED, GAME_STATUS.MAKEUP].includes(game.status)) {
    throw new Error(`GameEngine.commitGame: game ${gameIndex} is not in a committable state (${game.status})`);
  }
  ...
  StateManager.mutate(s => {
    ...
    // These are read by DashboardScreen's last-game badge and
    // ScheduleScreen's completed game rows.
    ...
    if (g.status === GAME_STATUS.FINAL || g.status === GAME_STATUS.MAKEUP) {
      if (won) {
        s.userTeam.wins++;
    ...
    // accumulateStats reads play._statDeltas and increments player.stats.
    // Must run before IMP recompute (which reads player.stats).
    if (g.plays && g.plays.length > 0) {
      ...
      // Apply stat deltas to each player
      for (const [playerId, delta] of Object.entries(statMutations.players || {})) {
        if (!s.players[playerId]) continue;
        const statKey = g.isSpring ? 'springStats' : 'stats';
        s.players[playerId][statKey] = {
          ...(s.players[playerId][statKey] || {}),
          ...(delta[statKey] || {}),
        };
      }

      // IMP game log — append per-player stat entries
      // appendGameLog expects the stat deltas from this game specifically
      for (const [playerId, delta] of Object.entries(statMutations.players || {})) {
        if (!s.players[playerId]) continue;
        const statKey = g.isSpring ? 'springStats' : 'stats';
        appendGameLog(s, playerId, gameIndex, delta[statKey] || {});
      }
    }
...
export async function advancePhase(newPhase) {
  ...
  switch (newPhase) {
    case PHASE.REGULAR_SEASON:
      // Spring cuts must be resolved before Opening Day
      // If still unresolved, auto-resolve by rating
      if (StateManager.get().userTeam.rosterIds.length > ROSTER_LIMITS.ACTIVE_TOTAL) {
        const cutMutations = autoResolveSpringCuts(StateManager.get());
