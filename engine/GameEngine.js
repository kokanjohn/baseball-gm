...
export async function startNewGame(config) {
  ...
  // Generate schedules
  // Anchor the first spring training game to tomorrow at 1:05 PM local time.
  // This guarantees the first game is always in the future regardless of when
  // the user creates their team. The rest of the schedule flows forward from there.
  const tomorrow = new Date();
  tomorrow.setDate(tomorrow.getDate() + 1);
  tomorrow.setHours(0, 0, 0, 0); // midnight — _parseGameTime sets the actual hour

  const schedule        = generateUserSchedule(1, league.leagueTeams, tomorrow);
