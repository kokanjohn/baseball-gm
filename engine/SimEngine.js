...
function _buildLineup(rosterIds, players, isCPU) {
  const starterPlayers = rosterIds
    .map(id => players[id])
    .filter(p =>
      p &&
      !p.isInjured &&
  ...
  // Apply batting order: contact+speed leadoff, power cleanup, overall fills.
  // CPU teams use OVR sort (no sub-rating granularity needed for CPU).
  let orderedStarters;
  if (isCPU) {
    orderedStarters = starterPlayers.sort((a,b) => b.ovr - a.ovr);
  } else {
    orderedStarters = _simDeriveBattingOrder(starterPlayers);
  }
