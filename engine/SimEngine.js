...
export function generatePlays(game, userTeam, opponentTeam, players, context) {
  ...
  // Show only the away team's SP (who pitches first), with a note about home SP
  const awayName = awaySPPlayer?.name || 'SP';
  const homeTeam = game.isHome
    ? (userSPPlayer?.name ? `${userSPPlayer.name} takes the mound at home.` : '')
    : '';
  const announcement = game.isHome
    ? `${awayName} starts for the visitors. ${userSPPlayer?.name || ''} takes the mound for your team.`
    : `${awayName} takes the mound.`;
