...
export function generatePlays(game, userTeam, opponentTeam, players, context) {
  ...
  const oppLineup  = opponentTeam
    ? _buildLineup(opponentTeam.rosterIds, players, true)
    : _buildCPULineup(opponentTeam, players);

  // Select starting pitchers from rotation
  const userSP  = _getRotationSP(userTeam, players);
  const oppSP   = opponentTeam
    ? _getRotationSP(opponentTeam, players)
    : _getCPUSP(opponentTeam, players);
  ...
  const gameState = _simulateFullGame(
    ...
    userSP, oppSP,
    players,
    { game, fieldMod, scoringMod, isHome: game.isHome, phase }
  );

  // Assign real-world timestamps to each play
  const plays = _assignTimestamps(gameState.plays, game.gameTime, game._tickOffset || 0);
...
function _simulateFullGame(userLineup, oppLineup, userSP, oppSP, players, ctx) {
  const { game, fieldMod, scoringMod, isHome, phase } = ctx;
  ...
  const gs = {
    ...
    userScore:    0,
    oppScore:     0,
    userLineupIdx:  0,  // batting order position
    oppLineupIdx:   0,
    userPitcher:  userSP,
    oppPitcher:   oppSP,
    userPitchCount: 0,
  ...
  while (!_gameOver(gs)) {
    ...
    const topBatter  = userBatsBottom ? oppLineup  : userLineup;
    ...
    const topPlays = _simulateHalfInning(
      topBatter, topPitcher, gs, players, fieldMod, topIsUser,
      `TOP_${gs.inning}`, topBatIdx
    );
    plays.push(...topPlays);
    ...
    // Update cumulative scores on top plays
    _updateCumulativeScores(topPlays, gs.userScore, gs.oppScore, isHome);

    // Check for pitcher change after half-inning
    _checkPitcherChange(gs, players, userBatsBottom ? 'user' : 'opp', userLineup, oppLineup, plays, gs.inning);

    // Walk-off check: if home team leads after 8.5+ innings, game ends
    if (gs.inning >= 9 && !userBatsBottom && gs.userScore > gs.oppScore) break;

    // Bottom of inning — home team bats
    const botBatter  = userBatsBottom ? userLineup  : oppLineup;
    ...
    const botPlays = _simulateHalfInning(
      botBatter, botPitcher, gs, players, fieldMod, botIsUser,
      `BOT_${gs.inning}`, botBatIdx
    );
    plays.push(...botPlays);
    ...
    // Pitcher change check after bottom
    _checkPitcherChange(gs, players, userBatsBottom ? 'opp' : 'user', userLineup, oppLineup, plays, gs.inning);

    gs.inning++;

    // Extra innings — cap at 12
    if (gs.inning > 12 && gs.userScore !== gs.oppScore) break;
    if (gs.inning > 13) break; // tie resolved by last score
  }

  // Apply scoring modifier (weather/field effects)
  if (scoringMod !== 1.0) {
  ...
  plays.push({
    ...
    pitcherId:    null,
    type:         'game_end',
    description:  gs.userScore > gs.oppScore ? 'Final' : 'Final',
    rbi:          0,
    isScoring:    false,
    isBigPlay:    false,
    outsAfter:    3,
...
function _simulateHalfInning(lineup, pitcherObj, gs, players, fieldMod, isUserBatting, halfKey, batIdxKey) {
  const plays = [];
  let outs  = 0;
  ...
  while (outs < 3) {
    const batterEntry = lineup[gs[batIdxKey] % lineup.length];
    const batter      = batterEntry ? players[batterEntry.id] : null;
    gs[batIdxKey]++;
    ...
    const atBat = simulateAtBat(batter, pitcher, bases, outs, players, fieldMod);
    gs[isUserBatting ? 'userPitchCount' : 'oppPitchCount']++;

    // Advance bases and score runners
    const { newBases, runsScored, runnerIds } = _advanceBases(bases, atBat, batter, players);
    bases = newBases;

    // Build rbi count
    let rbi = runsScored;
    // HR always counts itself
    if (atBat.type === 'hr') rbi = runsScored; // already includes self
    ...
    // Stat deltas
    const deltas = _buildStatDeltas(atBat, batter, pitcher, rbi, runsScored, runnerIds);

    plays.push({
      playIndex:    0, // renumbered after full game
      _halfInning:  halfKey,
      ...
      type:         atBat.type,
      description:  _buildDescription(atBat, batter, pitcher),
      rbi,
      isScoring,
      isBigPlay,
      outsAfter:    _isOut(atBat.type) ? outs + 1 : outs,
      _basesAfter:  { ...bases },
      cumOurScore:   0, // filled after full game
      cumTheirScore: 0,
      _timestamp:   0, // assigned later
      _statDeltas:  deltas,
    });
    ...
    // Check fatigue after each batter
...
function _buildBoxScore(plays, userLineup, oppLineup, players) {
  ...
  for (const play of plays) {
    if (!play.inning || play.type === 'inning_end' || play.type === 'game_end') continue;
...
function _checkPitcherChange(gs, players, team, userLineup, oppLineup, plays, inning) {
  const isUser    = team === 'user';
  const fatigued  = isUser ? gs.userFatigued : gs.oppFatigued;
  if (!fatigued) return;

  const bullpen    = isUser
    ? userLineup.filter(p => {
...
function _gameOver(gs) {
  ...
  return gs.inning >= 13; // extra innings cap
}
