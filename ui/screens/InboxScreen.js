  ...
  const bodyHtml = isExpanded ? `
    <div class="msg-body open">
      ...
      <div class="msg-actions">
        <button class="btn-yes" data-resolve="${card.instanceId}" data-choice="yes">
          ${_escape(card.yesLabel || 'Yes')}
        </button>
        <button class="btn-no"  data-resolve="${card.instanceId}" data-choice="no">
...
function _renderCompose(state) {
  ...
  const fieldStaff  = COMPOSE_CHANNELS.filter(c => c.role === 'Field Staff');
  const frontOffice = COMPOSE_CHANNELS.filter(c => c.role === 'Front Office');
  ...
  const renderGroup = (label, channels) => `
    <div style="margin-bottom:16px;">
      <div style="font-size:10px;font-weight:700;letter-spacing:2px;text-transform:uppercase;
        color:var(--muted);margin-bottom:8px;">${label}</div>
      <div class="compose-recipient-grid">
        ${channels.map(ch => {
          const onCooldown = _isOnCooldown(ch, cooldowns, gameIdx);
          return `
            <div class="compose-recipient-btn ${onCooldown ? '' : ''}"
              data-channel="${ch.id}"
              ${onCooldown ? 'data-cooldown="true"' : ''}>
              <div class="compose-recipient-icon">${ch.icon}</div>
              <div class="compose-recipient-label">${ch.label}</div>
              ${onCooldown ? '<div style="font-size:9px;color:var(--muted);margin-top:2px;">Cooldown</div>' : ''}
            </div>
          `;
        }).join('')}
      </div>
  ...
  return `
    <div style="padding:12px 16px 16px;">
      <div style="font-size:13px;color:var(--muted);margin-bottom:16px;line-height:1.5;">
        Check in with your staff and front office between games.
        Topics on cooldown can be revisited after ${COMPOSE_COOLDOWN_GAMES} games.
      </div>
      ${renderGroup('Field Staff', fieldStaff)}
      ${renderGroup('Front Office', frontOffice)}
    </div>

    <!-- Topic picker — shown when a channel is selected -->
    <div id="compose-topic-panel" style="display:none;padding:0 16px 16px;">
      <div id="compose-topic-list"></div>
      <button class="btn-back" id="compose-back-btn" style="margin-top:8px;width:100%;">← Back</button>
    </div>
  `;
}
...
function _isOnCooldown(channel, cooldowns, gameIdx) {
  return channel.topics.every(t => {
    ...
    return lastUsed !== undefined && (gameIdx - lastUsed) < COMPOSE_COOLDOWN_GAMES;
  });
}

function _renderTopicList(channel, cooldowns, gameIdx) {
  const topics = channel.topics.map(topic => {
    const lastUsed   = cooldowns[topic.id];
    const onCooldown = lastUsed !== undefined && (gameIdx - lastUsed) < COMPOSE_COOLDOWN_GAMES;
    const gamesLeft  = onCooldown ? COMPOSE_COOLDOWN_GAMES - (gameIdx - lastUsed) : 0;

    return `
      <button class="compose-topic-btn ${onCooldown ? 'disabled' : ''}"
        data-topic="${topic.id}" data-channel="${channel.id}"
        ${onCooldown ? 'disabled' : ''}>
        ${_escape(topic.label)}
        <span class="topic-desc">
          ${onCooldown
            ? `Available in ${gamesLeft} game${gamesLeft !== 1 ? 's' : ''}`
            : _escape(topic.desc)}
        </span>
      </button>
    `;
  ...
  return `
    <div style="font-size:11px;font-weight:700;letter-spacing:1px;text-transform:uppercase;
      color:var(--muted);margin-bottom:10px;">${channel.icon} ${channel.label}</div>
    <div class="compose-topic-list">${topics}</div>
  `;
}
...
function _attachTabListeners(state) {
  ...
  document.querySelectorAll('[data-channel]').forEach(el => {
    if (el.closest('#compose-topic-panel')) return; // topic panel buttons handled separately
    el.addEventListener('click', () => {
      if (el.dataset.cooldown) return;
      const channelId = el.dataset.channel;
      _showTopicPanel(channelId, state);
    });
  });

  // Compose back button
  const backBtn = document.getElementById('compose-back-btn');
  if (backBtn) backBtn.addEventListener('click', () => _hideTopicPanel());

  // Compose topic send
  document.querySelectorAll('.compose-topic-btn:not([disabled])').forEach(btn => {
    btn.addEventListener('click', async () => {
      const topicId   = btn.dataset.topic;
      const channelId = btn.dataset.channel;
      if (!topicId || !channelId) return;
      await _sendCompose(topicId, channelId, state);
    });
  });
}

function _showTopicPanel(channelId, state) {
  const channel      = COMPOSE_CHANNELS.find(c => c.id === channelId);
  if (!channel) return;

  const gameIdx    = state.currentGameIndex || 0;
  const cooldowns  = state.composeCooldowns || {};

  const topicPanel = document.getElementById('compose-topic-panel');
  const topicList  = document.getElementById('compose-topic-list');
  const recipGrid  = document.querySelector('.compose-recipient-grid');

  if (!topicPanel || !topicList) return;

  topicList.innerHTML = _renderTopicList(channel, cooldowns, gameIdx);
  topicPanel.style.display = 'block';

  // Re-attach topic listeners
  topicPanel.querySelectorAll('.compose-topic-btn:not([disabled])').forEach(btn => {
    btn.addEventListener('click', async () => {
      const topicId = btn.dataset.topic;
      if (!topicId) return;
      await _sendCompose(topicId, channelId, state);
...
async function _resolveCard(instanceId, choice, state) {
  try {
    ...
    EventBus.emit('card:resolved', { instanceId, choice });
    App.showToast(
      choice === 'yes' ? 'Decision made.' : 'Declined.',
      choice === 'yes' ? 'positive' : 'neutral',
      2000
    );

    refresh();
  } catch (err) {
    console.error('InboxScreen._resolveCard:', err);
