...
import { computeAge }    from '../../engine/PlayerFactory.js';
import {
...
function _openTeamModal(teamName, state) {
  ...
  // Wire player row taps — open player detail sheet
  overlay.querySelectorAll('.tm-player-row[data-player-id]').forEach(row => {
    row.addEventListener('click', (e) => {
      ...
      if (player) _openPlayerSheet(player, playerId, state);
    });
  });
}

// ─────────────────────────────────────────────────────────────
// PLAYER DETAIL SHEET — shared across schedule, team modal, etc.
// ─────────────────────────────────────────────────────────────

function _openPlayerSheet(player, playerId, state) {
  document.getElementById('sched-player-sheet')?.remove();

  const impScores = state.impScores || {};
  const imp       = impScores[playerId];
  const imp7      = imp?.imp7;
  const impSign   = imp7 > 0 ? '+' : '';
  const impColor  = imp7 > 0 ? 'var(--accent2)' : imp7 < 0 ? 'var(--danger)' : 'var(--muted)';

  const isSpring  = state.phase === 'SPRING_TRAINING';
  const statKey   = isSpring ? 'springStats' : 'stats';
  const stats     = player[statKey] || {};

  // Compute age from dob — player.age doesn't exist, only player.dob
  const age = player.dob ? computeAge(player.dob) : '—';

  const ovrColor = player.ovr >= 75 ? 'var(--accent2)'
    : player.ovr >= 60 ? 'var(--text)' : 'var(--muted)';

  // Sub-ratings — filter to non-null values only.
  // Pitchers have stuff/control/stamina set; contact/power/speed are null.
  // Hitters have contact/power/speed set; stuff/control/stamina are null.
  const subRatings = player.subRatings || {};
  const subRows = Object.entries(subRatings)
    .filter(([, val]) => val !== null && val !== undefined)
    .map(([k, val]) => {
      const col = val >= 70 ? '#22C55E' : val >= 50 ? '#F5D253' : '#f97316';
      return `<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
        <span style="font-size:11px;color:var(--muted);width:80px;flex-shrink:0;text-transform:capitalize;">${k.replace(/([A-Z])/g,' $1').trim()}</span>
        <div style="flex:1;height:4px;background:var(--border);border-radius:2px;overflow:hidden;">
          <div style="width:${val}%;height:100%;background:${col};border-radius:2px;"></div>
        </div>
        <span style="font-size:11px;font-weight:700;color:var(--text);width:24px;text-align:right;">${val}</span>
      </div>`;
    }).join('');

  // Season stats — hitters vs pitchers
  ...
  if (isPitcher) {
    ...
    statsHtml = `
      <div class="ps-stat-grid">
        <div class="ps-stat"><div class="ps-stat-val">${stats.wins || 0}</div><div class="ps-stat-lbl">W</div></div>
        <div class="ps-stat"><div class="ps-stat-val">${stats.losses || 0}</div><div class="ps-stat-lbl">L</div></div>
        <div class="ps-stat"><div class="ps-stat-val">${era}</div><div class="ps-stat-lbl">ERA</div></div>
        <div class="ps-stat"><div class="ps-stat-val">${ip}</div><div class="ps-stat-lbl">IP</div></div>
        <div class="ps-stat"><div class="ps-stat-val">${stats.k || 0}</div><div class="ps-stat-lbl">K</div></div>
        <div class="ps-stat"><div class="ps-stat-val">${stats.bb || 0}</div><div class="ps-stat-lbl">BB</div></div>
      </div>`;
  } else {
    const avg = (stats.ab || 0) > 0
      ? (stats.h / stats.ab).toFixed(3).replace('0.','.') : '—';
    const obp = (stats.ab || 0) > 0
      ? (((stats.h||0)+(stats.bb||0)) / Math.max((stats.ab||0)+(stats.bb||0), 1)).toFixed(3).replace('0.','.') : '—';
    statsHtml = `
      <div class="ps-stat-grid">
        <div class="ps-stat"><div class="ps-stat-val">${avg}</div><div class="ps-stat-lbl">AVG</div></div>
        <div class="ps-stat"><div class="ps-stat-val">${stats.hr || 0}</div><div class="ps-stat-lbl">HR</div></div>
        <div class="ps-stat"><div class="ps-stat-val">${stats.rbi || 0}</div><div class="ps-stat-lbl">RBI</div></div>
        <div class="ps-stat"><div class="ps-stat-val">${stats.r || 0}</div><div class="ps-stat-lbl">R</div></div>
        <div class="ps-stat"><div class="ps-stat-val">${obp}</div><div class="ps-stat-lbl">OBP</div></div>
        <div class="ps-stat"><div class="ps-stat-val">${stats.k || 0}</div><div class="ps-stat-lbl">K</div></div>
      </div>`;
  }

  const injuryBadge = player.isInjured
    ? `<span style="font-size:10px;font-weight:700;color:var(--danger);background:rgba(240,82,82,.1);border:1px solid rgba(240,82,82,.3);border-radius:6px;padding:2px 8px;">🏥 INJURED</span>`
    : '';

  const overlay = document.createElement('div');
  overlay.id = 'sched-player-sheet';
  overlay.className = 'modal-overlay';
  overlay.innerHTML = `
    <div class="modal-sheet" style="max-height:85dvh;">
      ...
      <div class="modal-header">
        <div>
          <div style="font-size:20px;font-weight:800;color:var(--text);line-height:1.1;">${_escape(player.name)}</div>
          <div style="font-size:12px;color:var(--muted);margin-top:3px;">${player.pos} · Age ${age}${injuryBadge ? ' · ' : ''}${injuryBadge}</div>
        </div>
        <div style="display:flex;flex-direction:column;align-items:flex-end;gap:6px;">
          <button class="modal-close" id="player-sheet-close">×</button>
          <span style="font-family:'Bebas Neue',sans-serif;font-size:32px;line-height:1;color:${ovrColor};">${player.ovr}</span>
        </div>
      </div>
      <div class="modal-divider"></div>
      <div style="overflow-y:auto;padding:14px 20px;flex:1;padding-bottom:max(16px,env(safe-area-inset-bottom));">
        ${imp7 != null ? `<div style="font-size:12px;font-weight:600;color:${impColor};margin-bottom:12px;">Form: ${impSign}${imp7.toFixed(1)}</div>` : ''}
        ${statsHtml ? `<div style="font-size:10px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:var(--muted);margin-bottom:10px;">${isSpring ? 'Spring' : 'Season'} Stats</div>${statsHtml}` : ''}
        ${subRows ? `<div style="font-size:10px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:var(--muted);margin:14px 0 10px;">Ratings</div>${subRows}` : ''}
      </div>
    </div>`;

  document.body.appendChild(overlay);
  requestAnimationFrame(() => overlay.classList.add('open'));
  document.getElementById('player-sheet-close')?.addEventListener('click', () => overlay.remove());
  overlay.addEventListener('click', e => { if (e.target === overlay) overlay.remove(); });
  App.playSoundOpen();
}
...
function _injectCSS() {
  ...
  style.textContent = `
    ...
    /* Player sheet stats */
    .ps-stat-grid { display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:4px; }
    .ps-stat { background:var(--surface2);border-radius:10px;padding:10px 8px;text-align:center; }
    .ps-stat-val { font-family:'Bebas Neue',sans-serif;font-size:22px;line-height:1;color:var(--text); }
    .ps-stat-lbl { font-size:9px;font-weight:700;letter-spacing:1px;text-transform:uppercase;color:var(--muted);margin-top:2px; }
    /* Team modal player row hover */
    .tm-player-row:active { background:var(--surface2); }
  `;
