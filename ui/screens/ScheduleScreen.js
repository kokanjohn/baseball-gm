...
function _renderGameRow(game, idx, currentIdx, state, isPlayoff) {
  ...
  const weatherIcon = _weatherIcon(game.weather?.label || game.weatherCondition);
...
function _renderResult(game, isCurrentGame, isCommitted) {
  ...
  const score  = game.score ? `${game.score.user}–${game.score.opponent}` : '';
...
function _scheduledTime(game) {
  if (!game._scheduledMs) return 'TBD';
  const d     = new Date(game._scheduledMs);
