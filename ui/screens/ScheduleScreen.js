...
function _openPlayerSheet(player, playerId, state) {
  ...
  const subKeys = Object.keys(subRatings).slice(0, 6);
  const subRows = subKeys.map(k => {
    const val = subRatings[k];
    const bar = Math.round((val / 100) * 100);
  ...
  // Season stats — show relevant stats based on position
  ...
  if (isPitcher) {
    ...
    const ops = (stats.ab || 0) > 0
      ? (((stats.h||0)+(stats.bb||0)) / (stats.ab||1) + ((stats.h||0)+(stats.hr||0)*3+(stats.doubles||0)+(stats.triples||0)*2) / Math.max(stats.ab,1)).toFixed(3)
      : '—';
    statsHtml = `
      <div class="ps-stat-grid">
        ...
        <div class="ps-stat"><div class="ps-stat-val">${stats.sb || 0}</div><div class="ps-stat-lbl">SB</div></div>
        <div class="ps-stat"><div class="ps-stat-val">${ops}</div><div class="ps-stat-lbl">OPS</div></div>
      </div>`;
  }
    ...
    <div class="modal-sheet" style="max-height:85dvh;">
      ...
      <div style="padding:16px 20px 12px;border-bottom:1px solid var(--border);">
        <div style="display:flex;align-items:flex-start;justify-content:space-between;">
          <div>
            <div style="font-size:20px;font-weight:800;color:var(--text);">${_escape(player.name)}</div>
            <div style="font-size:12px;color:var(--muted);margin-top:2px;">${player.pos} · Age ${player.age || '?'} · ${isSpring ? 'Spring' : 'Season'} Stats</div>
          </div>
        ...
        ${imp7 != null ? `<div style="margin-top:8px;font-size:12px;font-weight:600;color:${impColor};">Form: ${impSign}${imp7.toFixed(1)}</div>` : ''}
      </div>
