...
async function _render() {
  ...
  const isPremium    = false; // Phase 18 gates this — always false for now
  const slotLimit    = isPremium ? Infinity : SAVE_SLOT_LIMIT_FREE;
  const atLimit      = !isPremium && slots.length >= slotLimit;
  ...
  container.innerHTML = `
    <div class="ts-wrap">
      ...
      <div class="ts-new-card ${atLimit ? 'at-limit' : ''}" id="ts-new-btn">
        <div class="ts-new-plus">+</div>
        <div class="ts-new-label">New Team</div>
      </div>

      ${atLimit ? `
        <div class="ts-limit-note">
          Free tier: 1 team. Starting a new team will replace your current save.
        </div>
      ` : ''}

      ${isPremium ? `
        <div class="ts-slot-count">${slots.length} of unlimited teams</div>
      ` : ''}

    </div>
  `;

  _attachListeners(slots, activeSlotId, atLimit);
}
...
function _renderSlotCard(slot, activeSlotId) {
  ...
  const tierName     = state.prestigeTierName  || _tierName(tier);
  ...
  return `
    <div class="ts-card ${isActive ? 'ts-card-active' : ''}" data-slot-id="${_esc(slot.slotId)}">
      ...
      <div class="ts-card-right">
        <div class="ts-tier-badge ${tierCls}">${tierName}${tier > 1 ? ' ' + tierStars : ''}</div>
...
function _attachListeners(slots, activeSlotId, atLimit) {
  ...
  if (newBtn) {
    newBtn.addEventListener('click', () => {
      if (atLimit && slots.length > 0) {
        _showNewTeamConfirm(slots[0]);
      } else {
        _onNewTeam?.();
      }
    });
...
function _showNewTeamConfirm(existingSlot) {
  _removeOverlay();
  ...
  overlay.innerHTML = `
    <div class="ts-confirm-sheet">
      <div class="ts-confirm-handle"></div>
      <div class="ts-confirm-icon">⚠️</div>
      <div class="ts-confirm-title">Replace your current team?</div>
      <div class="ts-confirm-body">
        Starting a new team will permanently delete<br>
        <strong>${_esc(existingSlot?.teamName || 'your current team')}</strong>.<br>
        This cannot be undone.
      </div>
      <div class="ts-confirm-actions">
        <button class="ts-confirm-cancel" id="ts-confirm-cancel">Cancel</button>
        <button class="ts-confirm-danger" id="ts-confirm-new">Start New Team</button>
      </div>
    </div>
  `;
  ...
  document.getElementById('ts-confirm-new')?.addEventListener('click', async () => {
    _removeOverlay();
    // Delete the existing slot then proceed to setup
    if (existingSlot?.slotId) {
      try {
        await StateManager.deleteSlot(existingSlot.slotId);
        localStorage.removeItem('bgm_activeSlotId');
      } catch (err) {
        console.warn('[TeamSelectScreen] pre-delete failed:', err);
      }
    }
    _onNewTeam?.();
  });
