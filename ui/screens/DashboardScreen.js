...
function _onTick({ play, game } = {}) {
  ...
  if (play && play.type !== 'inning_end' && play.type !== 'game_end'
      && play.type !== 'pitching_change' && play.type !== 'pinch_hit') {
    _prependPBPPlay(play);
  } else if (play && (play.type === 'inning_end')) {
    _prependPBPInningHeader(play);
  } else if (play && (play.type === 'pitching_change' || play.type === 'pinch_hit')) {
    // Show substitution plays in PBP with muted styling
    _prependPBPPlay(play);
  }
...
function _deriveGameState(game) {
  ...
  // When outs = 3 the half-inning is complete — show "END" not "3 OUTS"
  // (the inning_end play will arrive on next tick and flip the half display)
  const outsLabel = isFinal ? '—'
    : outs >= 3             ? 'END'
    : `${outs} OUT${outs !== 1 ? 'S' : ''}`;
...
function _renderLinescore(ds, awayAbbr, homeAbbr) {
  ...
  // Fall back to US/OPP if abbrs not passed (e.g. from _onTick)
...
function _renderBoxScore(ds, game, state) {
  ...
  // Build starting lineup from rotation — all starters shown from first pitch,
  // even before they've had an AB. Bench pinch hitters appear when they play.
  const starterIds = (state?.userTeam?.rosterIds || [])
    .filter(id => {
      const p = players[id];
      return p && p.group === PLAYER_GROUP.STARTING_HITTERS && !p.isInjured;
    })
