...
function _deriveGameState(game) {
  ...
  for (const p of revealedPlays) {
    if (p.type === 'inning_end' || p.type === 'game_end'
        || p.type === 'pitching_change' || p.type === 'pinch_hit') continue;
    if (p.batterId) {
...
function _renderBoxScore(ds) {
  ...
  // User team players only, sorted by first appearance (= batting order)
  const batEntries = Object.entries(ds.battersBox)
    .filter(([id]) => userIds.has(id))
    .map(([id,s]) => ({ id, p: players[id], s, firstIdx: ds.batterFirstIdx?.[id] ?? 999 }))
    .filter(e => e.p)
  ...
  const batRows = batEntries.map(({p,s}) => {
    const isBench = p.group === 'bh'; // BENCH_HITTERS
