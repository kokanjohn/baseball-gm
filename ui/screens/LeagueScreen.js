...
function _renderPlayers(state) {
  ...
  // "On Rosters" = hide user's own players, show only CPU roster players
  ...
  // Position filter — families: 2B includes 2B/SS; 1B includes 1B/3B; DH includes DH/OF
  ...
  const statCols = isPitcherView
    ? [{ key:'ovr',label:'OVR' },{ key:'era',label:'ERA' },{ key:'ip',label:'IP' },{ key:'k',label:'K' },{ key:'bb',label:'BB' },{ key:'svhd',label:'SV+H' }]
    : [{ key:'ovr',label:'OVR' },{ key:'avg',label:'AVG' },{ key:'hr',label:'HR' },{ key:'rbi',label:'RBI' },{ key:'r',label:'R' },{ key:'sb',label:'SB' }];

  const allCols = [{ key:'imp7', label:'Form' }, ...statCols];
...
function _injectCSS() {
  ...
  style.textContent = `
    ...
    .lp-team-select{width:90px;flex-shrink:0;font-size:11px;font-weight:500;
      background:var(--surface);border:1px solid var(--border);border-radius:7px;
      color:var(--text);padding:4px 6px;font-family:'DM Sans',sans-serif;outline:none;}
    .lp-search-input{flex:1;min-width:0;padding:4px 8px;border-radius:7px;
