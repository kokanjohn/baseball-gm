...
export function mount() {
  ...
  _listeners.push(EventBus.on('game:committed',    () => { if (_activeTab === 'standings') refresh(); }));
  _listeners.push(EventBus.on('game:phaseChanged', () => refresh()));
  _listeners.push(EventBus.on('nav:tabActivated',  ({ tab }) => { if (tab === 'league') refresh(); }));
...
function _renderStandings(state) {
  ...
  const renderDiv = (label, teams) => {
    ...
    const rows = teams.map((entry, idx) => {
      ...
      return `
        <div class="standing-row ${isUser ? 'player-row' : ''}">
...
function _renderStatLeaders(state) {
  ...
  const hitters  = allPlayerIds.map(id => players[id]).filter(p => p && !['SP','RP'].includes(p.pos) && (p.stats?.ab || 0) >= 30);
  const pitchers = allPlayerIds.map(id => players[id]).filter(p => p && ['SP','RP'].includes(p.pos) && (p.stats?.ip || 0) >= 5);
...
function _renderPlayers(state) {
  ...
  const teamChips = [
    { val: 'all',  label: 'All Teams' },
    { val: 'user', label: state.userTeam?.abbr || 'My Team' },
  ].map(c => `
    <div class="lp-chip ${_playerFilter.team === c.val ? 'lp-chip-active' : ''}" data-team="${c.val}">${c.label}</div>
  `).join('');

  return `
    <div id="lp-filter-wrap" class="lp-tab-wrap active" style="display:flex;flex-direction:column;">
      <div class="lp-filter-bar">
        <div class="lp-filter-row">${posChips}</div>
        <div class="lp-filter-row">${teamChips}</div>
      </div>
      <div class="lp-header">
