...
function _renderPlayers(state) {
  ...
  const rows = allPlayers.slice(0, 100).map(p => {
    ...
    const statVals = statCols.map(c => {
      ...
      return `<div class="lp-td">${_playerStatVal(p, c.key)}</div>`;
    }).join('');
...
function _injectCSS() {
  ...
  style.textContent = `
    ...
    /* Header and rows use the same grid-like flex so columns align */
    ...
    /* Sticky name column — always visible on the left */
    .lp-col-sticky, .lp-td-name{
    ...
    .lp-col-sticky{background:var(--surface2);padding:5px 8px;}

    /* Stat columns — uniform narrow width */
    .lp-th{min-width:36px;width:36px;text-align:center;padding:5px 4px;
      font-size:9px;font-weight:700;letter-spacing:.5px;text-transform:uppercase;
      color:var(--muted);cursor:pointer;flex-shrink:0;}
    .lp-th:active{opacity:.7;}
    .lp-th-active{color:var(--accent);background:var(--chip-accent-bg);}
    .lp-td{min-width:36px;width:36px;text-align:center;padding:8px 4px;
      font-size:11px;color:var(--soft);flex-shrink:0;}
    .lp-td-imp{min-width:40px;width:40px;}

    /* H/AB is wider since it's two numbers */
    .lp-th[data-sort="hab"], .lp-td:nth-child(2){min-width:44px;width:44px;}
