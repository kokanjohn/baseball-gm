...
function _renderStandings(state) {
  ...
  const renderDiv = (label, teams) => {
    ...
    const rows = teams.map((entry, idx) => {
      ...
      return `
        <div class="standing-row ${isUser ? 'player-row' : ''}" data-team-id="${entry.id}" style="cursor:pointer;">
...
function _renderPlayers(state) {
  ...
  const rosterFilterBar = `
    <div class="lp-roster-bar">
      ...
      <button class="lp-reset-btn" id="lp-reset-btn" title="Reset filters">↺</button>
    </div>
  `;

  // Column definitions — order per spec.
  // Hitters:  H/AB | AVG | R | HR | RBI | BB | SB | IMP | RTG
  // Pitchers: IP | H | ER | BB | K | W | ERA | SVHD | IMP | RTG
  // The first 2 visible stat cols (before scroll) are: AVG for hitters, IP for pitchers.
  // IMP and RTG are last — scroll into view.
  ...
  // IMP is now inline in statCols — no separate allCols needed
  const heads = statCols.map(c => {
    const isActive = c.key === _playerSort.col;
    const extraCls = c.key === 'hab' ? ' lp-th-hab' : c.key === 'imp7' ? ' lp-td-imp' : '';
    return `<div class="lp-th${extraCls} ${isActive ? 'lp-th-active' : ''}" data-sort="${c.key}">${c.label}</div>`;
  }).join('');

  const rows = allPlayers.slice(0, 100).map(p => {
    const isUser   = userRosterIds.has(p.id);
    const imp      = impScores[p.id];
    // Use team abbr for user's players — never 'YOU'
    const teamAbbr = isUser
      ? ((state.userTeam?.abbr || '').toUpperCase() || '—')
      : _teamAbbr(p.teamId, state);
    const statVals = statCols.map(c => {
      if (c.key === 'imp7') {
        const imp7 = imp?.imp7;
        return `<div class="lp-td lp-td-imp">${
          imp7 != null
      ...
      const extraCls = c.key === 'hab' ? ' lp-td-hab' : '';
      return `<div class="lp-td${extraCls}">${_playerStatVal(p, c.key)}</div>`;
    }).join('');

    return `
      <div class="lp-row ${isUser ? 'user-player-row' : ''}" data-player="${p.id}">
        <div class="lp-td-name">
          <div class="lp-name-line">${_escape(p.name)}</div>
  ...
  return `
    <div id="lp-filter-wrap" class="lp-tab-wrap active">
      ...
      <div class="lp-scroll-wrap">
        <div class="lp-header">
          <div class="lp-th lp-th-name lp-col-sticky">Player</div>
          ${heads}
        </div>
...
function _playerStatVal(player, key) {
  ...
  switch (key) {
    ...
    case 'hab':  return s.ab ? `${s.h||0}/${s.ab}` : '0/0';
    ...
    case 'h_allowed': return String(s.h_allowed || 0);
    case 'er':   return String(s.er  || 0);
    case 'era':  {
...
function _sortVal(player, col) {
  ...
  switch (col) {
    ...
    case 'hab':  return s.ab ? (s.h||0) / s.ab : 0; // sort by avg for H/AB
    case 'avg':  return s.ab ? (s.h||0) / s.ab : 0;
    case 'hr':   return s.hr  || 0;
    case 'rbi':  return s.rbi || 0;
    case 'r':    return s.r   || 0;
    case 'sb':   return s.sb  || 0;
    case 'h_allowed': return s.h_allowed || 0;
    case 'er':   return s.er  || 0;
...
function _attachListeners(state) {
  ...
  // Reset button — clears all filters, search, and sort back to defaults
  document.getElementById('lp-reset-btn')?.addEventListener('click', () => {
    App.playClick();
    _playerFilter = { pos: 'batters', team: 'all', onRosters: false };
    _playerSort   = { col: 'ovr', dir: 'desc' };
    _lpSearch     = '';
    refresh();
  });
  ...
  // Standings tab — standing row click → team roster modal
  document.querySelectorAll('.lg-stand-row[data-team-id], .standing-row[data-team-id]').forEach(el => {
    el.addEventListener('click', () => {
      const teamId = el.dataset.teamId;
      if (!teamId) return;
      const s = StateManager.get();
      if (teamId === 'user') {
        _openLeagueTeamModal(s.userTeam, s);
      } else {
        const team = (s.leagueTeams || []).find(t => t.id === teamId);
        if (team) _openLeagueTeamModal(team, s);
      }
...
function _openLeagueTeamModal(team, state) {
  ...
  const players   = state.players || {};
  const isUser    = team.id === 'user' || !team.id;

  const hitters  = rosterIds.map(id => players[id])
    .filter(p => p && !['SP','RP'].includes(p.pos)).sort((a,b) => b.ovr - a.ovr);
  const pitchers = rosterIds.map(id => players[id])
    .filter(p => p && ['SP','RP'].includes(p.pos)).sort((a,b) => b.ovr - a.ovr);

  const avgOvr = rosterIds.length
    ? Math.round(rosterIds.map(id => players[id]?.ovr || 0).reduce((a,b) => a+b, 0) / rosterIds.length)
    : 0;

  const playerRow = (p) => `
    <div class="lgt-player-row" data-player-id="${p.id}"
      style="display:flex;align-items:center;gap:10px;padding:10px 20px;
             border-bottom:1px solid var(--border);cursor:pointer;">
      <span style="flex:1;font-size:13px;font-weight:600;color:var(--text);">${_escape(p.name)}</span>
      <span style="font-size:11px;color:var(--muted);margin-right:4px;">${p.nativePos || p.pos}</span>
      <span style="font-size:13px;font-weight:700;min-width:26px;text-align:right;
        color:${p.ovr >= 75 ? 'var(--accent2)' : p.ovr >= 60 ? 'var(--text)' : 'var(--muted)'};">${p.ovr}</span>
      <span style="font-size:11px;color:var(--border);margin-left:2px;">›</span>
    </div>`;

  const sectionHead = (label) =>
    `<div style="padding:8px 20px 4px;font-size:10px;font-weight:700;letter-spacing:2px;
      text-transform:uppercase;color:var(--muted);background:var(--surface2);">${label}</div>`;

  const overlay = document.createElement('div');
  overlay.id = 'league-team-modal';
  overlay.className = 'modal-overlay';
  overlay.innerHTML = `
    <div class="modal-sheet" style="max-height:86dvh;display:flex;flex-direction:column;">
      ...
      <div class="modal-header" style="flex-shrink:0;">
        <div>
          ...
          <div style="font-size:12px;color:var(--muted);margin-top:2px;">
            ${team.wins || 0}–${team.losses || 0} · Avg OVR ${avgOvr}
            ${isUser ? ' · <span style="color:var(--accent);font-weight:700;">Your Team</span>' : ''}
          </div>
        </div>
        <button class="modal-close" id="league-team-close">×</button>
      </div>
      <div class="modal-divider" style="flex-shrink:0;"></div>
      <div style="overflow-y:auto;flex:1;padding-bottom:max(16px,env(safe-area-inset-bottom));">
        ${hitters.length  ? sectionHead('Hitters')  + hitters.map(playerRow).join('')  : ''}
        ${pitchers.length ? sectionHead('Pitchers') + pitchers.map(playerRow).join('') : ''}
      </div>
    </div>`;

  document.body.appendChild(overlay);
  requestAnimationFrame(() => overlay.classList.add('open'));
  overlay.querySelector('#league-team-close')?.addEventListener('click', () => overlay.remove());
  overlay.addEventListener('click', e => { if (e.target === overlay) overlay.remove(); });

  // Wire player row taps → PlayerCard
  overlay.querySelectorAll('.lgt-player-row[data-player-id]').forEach(row => {
...
function _injectCSS() {
  ...
  style.textContent = `
    ...
    .lp-reset-btn{flex-shrink:0;width:28px;height:28px;border-radius:7px;
      border:1px solid var(--border);background:transparent;color:var(--muted);
      font-size:16px;cursor:pointer;display:flex;align-items:center;justify-content:center;
      font-family:'DM Sans',sans-serif;padding:0;}
    .lp-reset-btn:active{opacity:.6;}

    /* Horizontal scroll wrapper — name col sticks left, stats scroll right */
    .lp-tab-wrap{display:block;}
    .lp-sticky-header{position:sticky;top:0;z-index:10;background:var(--bg);}
    .lp-scroll-wrap{overflow-x:auto;-webkit-overflow-scrolling:touch;}
    .lp-scroll-wrap::-webkit-scrollbar{height:3px;}
    .lp-scroll-wrap::-webkit-scrollbar-thumb{background:var(--border);border-radius:2px;}

    /* Header and rows — flex with same widths so columns align */
    .lp-header{display:flex;align-items:center;border-bottom:2px solid var(--border);
      background:var(--surface2);min-width:max-content;}
    .lp-row{display:flex;align-items:center;border-bottom:1px solid var(--border);
      cursor:pointer;min-width:max-content;}
    .lp-row:active{background:var(--surface2);}

    /* Sticky name column — must be identical width in both header and rows */
    .lp-col-sticky{
      position:sticky;left:0;z-index:3;
      width:120px;min-width:120px;max-width:120px;flex-shrink:0;
      background:var(--surface2);padding:5px 8px;
    }
    .lp-td-name{
      position:sticky;left:0;z-index:2;
      width:120px;min-width:120px;max-width:120px;flex-shrink:0;
      background:var(--bg);padding:7px 8px;overflow:hidden;
    }
    .user-player-row .lp-td-name{background:var(--chip-accent-bg);}

    /* Player name text — 12px matching the previous fix */
    .lp-name-line{font-size:12px;font-weight:600;color:var(--text);
      white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
    .lp-name-sub{font-size:10px;color:var(--muted);margin-top:1px;
      white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}

    /* Stat column headers — uniform 38px, text centered */
    .lp-th{width:38px;min-width:38px;flex-shrink:0;text-align:center;padding:5px 2px;
      font-size:9px;font-weight:700;letter-spacing:.3px;text-transform:uppercase;
      color:var(--muted);cursor:pointer;white-space:nowrap;}
    .lp-th:active{opacity:.7;}
    .lp-th-active{color:var(--accent);}

    /* Stat cells — match header widths exactly */
    .lp-td{width:38px;min-width:38px;flex-shrink:0;text-align:center;
      padding:8px 2px;font-size:11px;color:var(--soft);}

    /* H/AB needs extra width for two numbers */
    .lp-td-hab,.lp-th-hab{width:46px;min-width:46px;}

    /* IMP badge column */
    .lp-td-imp{width:44px;min-width:44px;}
