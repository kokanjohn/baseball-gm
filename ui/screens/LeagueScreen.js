...
function _renderPlayers(state) {
  ...
  // "On Rosters" filter — hide free agents (players without a team)
  if (_playerFilter.onRosters) {
    allPlayers = allPlayers.filter(p => p.teamId || userRosterIds.has(p.id));
  }
  ...
  if (pos === 'batters') {
    ...
    // Specific position
    allPlayers = allPlayers.filter(p => (p.nativePos || p.pos) === pos);
  }
  ...
  // Position filter bar — Batters | Pitchers | C 1B 2B 3B SS OF DH | SP RP
  ...
  // Team dropdown + All/On Rosters buttons
  const teamOptions = [
  ...
  const rosterFilterBar = `
    <div class="lp-roster-bar">
      ...
      <button class="lp-roster-btn ${_playerFilter.onRosters ? 'active' : ''}" data-roster="on">On Rosters</button>
      <select class="lp-team-select" id="lp-team-select">
        ${teamOptions}
      </select>
  ...
  // Columns — no separate TM column (team shown below player name)
  const cols = isPitcherView
    ? [{ key:'ovr',label:'OVR' },{ key:'era',label:'ERA' },{ key:'w',label:'W' },{ key:'k',label:'K' }]
    : [{ key:'ovr',label:'OVR' },{ key:'avg',label:'AVG' },{ key:'hr',label:'HR' },{ key:'rbi',label:'RBI' },{ key:'r',label:'R' },{ key:'sb',label:'SB' }];

  const heads = cols.map(c => `
    <div class="lp-th ${c.key === _playerSort.col ? 'lp-th-active' : ''}" data-sort="${c.key}">
      ${c.label}${c.key === _playerSort.col ? (_playerSort.dir === 'asc' ? '↑' : '↓') : ''}
    </div>`).join('');
  ...
  const rows = allPlayers.slice(0, 100).map(p => {
    ...
    const statVals  = cols.map(c => `<div class="lp-td">${_playerStatVal(p, c.key)}</div>`).join('');
    ...
    return `
      <div class="lp-row ${isUser ? 'user-player-row' : ''}" data-player="${p.id}">
        <div class="lp-td-name">
          ...
          <div class="lp-name-sub">${_escape(teamAbbr)} · ${p.pos}${impBadge ? ' ' : ''}${impBadge}</div>
        </div>
        ${statVals}
      </div>
    `;
...
function _injectCSS() {
  ...
  style.textContent = `
    ...
    /* Position filter bar */
    .lp-pos-bar{display:flex;flex-wrap:wrap;gap:3px;padding:7px 12px 5px;
      background:var(--surface);border-bottom:1px solid var(--border);}
    .lp-pos-btn{font-size:10px;font-weight:700;letter-spacing:.3px;padding:3px 7px;
    ...
    .lp-sep{color:var(--border);font-size:13px;line-height:1;align-self:center;margin:0 2px;}

    /* Roster filter bar (All / On Rosters / team dropdown / search) */
    .lp-roster-bar{display:flex;align-items:center;gap:6px;padding:6px 12px;
      background:var(--surface2);border-bottom:1px solid var(--border);flex-wrap:nowrap;}
    .lp-roster-btn{font-size:10px;font-weight:700;padding:4px 8px;border-radius:6px;
    ...
    .lp-team-select{flex:1;min-width:0;font-size:11px;font-weight:600;
      background:var(--surface);border:1px solid var(--border);border-radius:8px;
      color:var(--text);padding:4px 8px;font-family:'DM Sans',sans-serif;outline:none;}
    .lp-search-input{width:80px;flex-shrink:0;padding:4px 8px;border-radius:8px;
      border:1px solid var(--border);background:var(--surface);color:var(--text);
      font-size:11px;font-family:'DM Sans',sans-serif;outline:none;box-sizing:border-box;}

    /* Player rows — no separate TM column; team shown below name */
    ...
    .lp-imp{font-size:9px;font-weight:800;padding:1px 4px;border-radius:4px;
      letter-spacing:.2px;margin-left:4px;}
    .lp-imp-pos{color:var(--accent2);background:rgba(52,201,122,.1);}
    .lp-imp-neg{color:var(--danger);background:rgba(240,82,82,.1);}
    .lp-imp-neu{color:var(--muted);}
