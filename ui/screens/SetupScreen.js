...
const ARCHETYPES = [
  {
    ...
    name:     'The Ember',
    icon:     '🔥',
    premise:  'A once-great franchise, cold now. You\'re the rebuild.',
    roster:   '●●○○○',
    patience: '●●●●○',
    budget:   '●○○○○',
  },
  {
    ...
    name:     'The Lab',
    icon:     '🔬',
    premise:  'Analytics-forward. Process over results, for now.',
    roster:   '●●●○○',
    patience: '●●●●○',
    budget:   '●●○○○',
  },
  {
    ...
    name:     'The Contender',
    icon:     '🥊',
    premise:  'A good team that hasn\'t broken through. Playoff expectations.',
    roster:   '●●●●○',
    patience: '●●●○○',
    budget:   '●●●●○',
  },
  {
    ...
    icon:     '🏛️',
    premise:  'Old-school, veteran-heavy, deeply traditional.',
    roster:   '●●●●○',
    patience: '●●●○○',
    budget:   '●●●○○',
  },
  {
    ...
    name:     'The Gambler',
    icon:     '🎲',
    premise:  'A volatile franchise. Boom-or-bust everything.',
    roster:   '●●●○○',
    patience: '●●○○○',
    budget:   '●●●○○',
  },
  {
    ...
    name:     'The Empire',
    icon:     '👑',
    premise:  'A dynasty in progress. Championship or bust.',
    roster:   '●●●●●',
    patience: '●○○○○',
    budget:   '●●●●●',
  },
...
function _renderArchetypeStep() {
    ...
    return `
      <div class="${cls}" data-archetype="${a.id}" ${used ? 'aria-disabled="true"' : ''}>
        <div class="arch-header">
          ...
          ${used ? '<span class="arch-used-badge">IN USE</span>' : ''}
        </div>
        <p class="arch-premise">${a.premise}</p>
        <div class="arch-indicators">
          <div class="arch-ind-row">
            <span class="arch-ind-label">Roster</span>
            <span class="arch-ind-dots">${a.roster}</span>
          </div>
          <div class="arch-ind-row">
            <span class="arch-ind-label">Patience</span>
            <span class="arch-ind-dots">${a.patience}</span>
          </div>
          <div class="arch-ind-row">
            <span class="arch-ind-label">Budget</span>
            <span class="arch-ind-dots">${a.budget}</span>
          </div>
        </div>
      </div>
    `;
  ...
  return `
    <div class="intro-form">
      <div style="font-size:10px;letter-spacing:3px;text-transform:uppercase;color:var(--muted);font-weight:600;margin-bottom:10px;">
        Choose Your Franchise
      </div>
      <div class="archetype-grid">${cards}</div>
      <button class="btn-primary" id="arch-next-btn" ${!_archetype ? 'disabled' : ''}>
        Choose Franchise →
      </button>
    </div>
  `;
...
// ─────────────────────────────────────────────────────────────
// STEP 2 — REGION SELECTION (Phase 13.5)
// ─────────────────────────────────────────────────────────────

/**
 * _renderRegionStep()
 * Four-card grid — same visual pattern as archetype selection.
 * Region descriptions match plan Section 8.13 exactly.
 * Selected region stored in _region (defaults to REGION_DEFAULT).
 */
function _renderRegionStep() {
  ...
  const cards = regionEntries.map(r => {
    ...
    return `
      <div class="archetype-card${selected ? ' selected' : ''}" data-region="${r.id}">
  ...
  return `
    <div class="intro-form">
      <div style="text-align:center;margin-bottom:6px;">
        <span style="font-size:28px;">${arch?.icon || '⚾'}</span>
        <div style="font-size:11px;color:var(--accent);font-weight:700;letter-spacing:2px;text-transform:uppercase;margin-top:4px;">
          ${arch?.name || ''}
        </div>
      </div>
      ...
      <div>
        <div style="font-size:10px;letter-spacing:2px;text-transform:uppercase;color:var(--soft);font-weight:600;margin-bottom:8px;">
          Team Icon
        </div>
        <div class="icon-pick-grid" id="icon-grid">
          ${TEAM_ICONS.map(icon => `
            <div class="icon-pick-cell${_teamIcon === icon ? ' selected' : ''}" data-icon="${icon}">
              <div class="icon-pick-emoji">${icon}</div>
            </div>
          `).join('')}
        </div>
      </div>
...
function _attachListeners() {
  ...
  document.querySelectorAll('.archetype-card:not(.used)').forEach(card => {
    card.addEventListener('click', () => {
      _archetype = card.dataset.archetype;
  ...
  document.querySelectorAll('[data-region]').forEach(card => {
    card.addEventListener('click', () => {
      ...
      document.querySelectorAll('[data-region]').forEach(c => c.classList.remove('selected'));
      card.classList.add('selected');
    });
  });
  ...
  document.querySelectorAll('.icon-pick-cell').forEach(cell => {
    cell.addEventListener('click', () => {
      _teamIcon = cell.dataset.icon;
      document.querySelectorAll('.icon-pick-cell').forEach(c => c.classList.remove('selected'));
...
function _injectCSS() {
  ...
  style.textContent = `
    /* Archetype grid */
    .archetype-grid{display:flex;flex-direction:column;gap:8px;margin-bottom:16px;max-height:48dvh;overflow-y:auto;padding-right:2px;}
    .archetype-card{background:var(--surface);border:2px solid var(--border);border-radius:14px;
    ...
    .arch-header{display:flex;align-items:center;gap:8px;margin-bottom:6px;}
    .arch-icon{font-size:20px;line-height:1;}
    .arch-name{font-family:'Bebas Neue',sans-serif;font-size:18px;letter-spacing:2px;color:var(--text);flex:1;}
    .arch-used-badge{font-size:9px;font-weight:700;letter-spacing:.5px;padding:2px 6px;border-radius:4px;
      background:var(--surface2);color:var(--muted);border:1px solid var(--border);}
    .arch-premise{font-size:12px;color:var(--soft);line-height:1.4;margin-bottom:8px;}
    .arch-indicators{display:flex;flex-direction:column;gap:3px;}
    .arch-ind-row{display:flex;align-items:center;gap:8px;}
    .arch-ind-label{font-size:9px;font-weight:700;letter-spacing:1px;text-transform:uppercase;
      color:var(--muted);width:56px;flex-shrink:0;}
    .arch-ind-dots{font-size:12px;letter-spacing:2px;color:var(--accent);}
