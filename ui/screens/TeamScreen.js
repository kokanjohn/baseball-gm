...
async function _handleSwap(playerId, role, state) {
  ...
  if (role === 'bench') {
    ...
    const slotOccupants = eligibleSlots.map(sl => {
      const idx     = lineupSlots.findIndex(s => s.slot === sl && s.playerId);
      const occupant = idx >= 0 ? StateManager.get().players[lineupSlots[idx].playerId] : null;
      return { slot: sl, slotIdx: idx, occupant };
    }).filter(s => s.slotIdx >= 0 || eligibleSlots.includes(s.slot));
