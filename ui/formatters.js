...
export function applyTeamColors(primaryId, secondaryId, theme) {
  ...
  const primaryEntry   = _findColor(primaryId   || COLOR_PRIMARY_DEFAULT);
  const secondaryEntry = _findColor(secondaryId  || COLOR_SECONDARY_DEFAULT);

  const primaryHex   = primaryEntry[activeTheme]   || primaryEntry.dark;
  const secondaryHex = secondaryEntry[activeTheme]  || secondaryEntry.dark;
...
export function getColorHex(colorId, theme) {
  const activeTheme = theme
    ...
    || 'dark';
  const entry = _findColor(colorId);
  return entry[activeTheme] || entry.dark;
}
