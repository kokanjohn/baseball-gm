...
export async function init() {
  ...
  if (gearBtn && settingsModal) {
    gearBtn.addEventListener('click', () => {
      ...
      // Re-initialize every time settings opens so colors/info are current
...
function _initSettingsPickers() {
  ...
  document.getElementById('settings-reset-row')?.addEventListener('click', () => {
    if (!confirm('Start a new franchise? This will erase all progress and cannot be undone.')) return;
    // Clear the active slot and reload
    StateManager.clearActiveSlot?.();
    localStorage.clear();
