...
/**
 * _refreshSettingsSwatches()
 * Called every time settings opens. Reads current state and updates:
 * - Franchise info (GM name, team, season)
 * - Color swatches (selected state reflects actual saved colors)
 * - Color preview dots
...
function _refreshSettingsSwatches() {
  ...
  // Franchise info
  const gmNameEl = document.getElementById('settings-gm-name');
  if (gmNameEl) gmNameEl.textContent = state.gmName || '—';
  ...
  // Primary swatches — update selected class only (innerHTML already set by _initSettingsPickers)
  document.querySelectorAll('.settings-swatch[data-target="primary"]').forEach(s => {
    s.classList.toggle('selected', s.dataset.colorId === currentPrimary);
    // Also update background in case theme changed
    const c = palette.find(p => p.id === s.dataset.colorId);
    if (c) s.style.background = c[activeTheme] || c.dark;
  });

  document.querySelectorAll('.settings-swatch[data-target="secondary"]').forEach(s => {
    s.classList.toggle('selected', s.dataset.colorId === currentSecondary);
    const c = palette.find(p => p.id === s.dataset.colorId);
    if (c) s.style.background = c[activeTheme] || c.dark;
  });
...
/**
 * _initSettingsPickers()
 * Called ONCE during init(). Populates swatch grids and wires all
 * event listeners. Never called again — listeners persist on the DOM.
 */
