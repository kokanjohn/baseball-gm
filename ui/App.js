...
export const APP_VERSION = 'tfo-v2-r52';
...
export function updateHeader(state) {
  ...
  if (tierEl) {
    // Tier label removed from header — displayed on Team screen instead (Tier X of 5).
    // Hide the element but keep it in the DOM so layout doesn't shift.
    tierEl.textContent = '';
...
export async function init() {
  ...
  // Wire milestone overlay — populate content and handle Continue tap
  // showMilestone() emits 'nav:milestone' with the full milestone definition object.
  EventBus.on('nav:milestone', ({ milestoneId }) => {
    // milestoneId is actually the full milestone definition object from MILESTONE_SCREENS
    const m = milestoneId;
    if (!m) return;
    const titleEl    = document.getElementById('milestone-title');
    const subtitleEl = document.getElementById('milestone-subtitle');
    const trophyEl   = document.getElementById('milestone-trophy');
    if (titleEl)    titleEl.textContent    = m.title    || 'Milestone';
    if (subtitleEl) subtitleEl.textContent = m.subtitle || '';
    if (trophyEl)   trophyEl.textContent   = m.icon     || '🏆';
  });

  // Continue button — dismiss milestone and resume normal navigation
  document.getElementById('milestone-continue')?.addEventListener('click', () => {
    clearMilestone();
    // Clear pendingMilestone from state if it's still set
    const state = StateManager.get();
...
export function startTick() {
  ...
  _tickInterval = setInterval(async () => {
    ...
    // Re-read game after tick() — status may have just changed to FINAL
    const freshGame = StateManager.get()?.schedule?.[gameIdx];

    // Auto-commit when game reaches FINAL
    if (freshGame?.status === GAME_STATUS.FINAL && !freshGame._committed) {
      try {
...
// Fast-forwards live game state silently when the app resumes
// from background so the user sees the correct game state
// immediately rather than watching plays catch up in real time.
// ─────────────────────────────────────────────────────────────
...
document.addEventListener('visibilitychange', async () => {
  ...
  // Resuming — check how much time elapsed
  const elapsed = Date.now() - _lastActiveTime;
  if (elapsed < TICK_INTERVAL_MS * 2) return; // less than 2 ticks — nothing to catch up

  const state   = StateManager.get();
  const gameIdx = state?.currentGameIndex ?? 0;
  const game    = state?.schedule?.[gameIdx];

  // Only fast-forward if a game is currently live and not yet committed
  if (!game || game.status === GAME_STATUS.FINAL || game._committed) return;
  if (!game.plays || game.livePlayIndex === undefined) return;

  const totalPlays   = game.plays.length;
  const currentIndex = game.livePlayIndex ?? 0;
  if (currentIndex >= totalPlays - 1) return; // already at end

  // Calculate how many ticks should have elapsed
  const ticksElapsed = Math.floor(elapsed / TICK_INTERVAL_MS);
  if (ticksElapsed <= 0) return;

  // Fast-forward: advance plays silently without emitting per-play events
  const { tick, commitGame } = await import('../engine/GameEngine.js');
  ...
  for (let i = 0; i < ticksElapsed; i++) {
    ...
    // Check if game reached FINAL during fast-forward
    const freshGame = StateManager.get()?.schedule?.[gameIdx];
    if (freshGame?.status === GAME_STATUS.FINAL) {
      if (!freshGame._committed) {
        try {
          const result = await commitGame(gameIdx);
          EventBus.emit('game:committed', { result, gameIndex: gameIdx });
          if (result?.milestone) showMilestone(result.milestone);
        } catch (err) {
          console.error('App visibilitychange: auto-commit error:', err);
        }
      }
      _lastActiveTime = Date.now();
      return;
  ...
  // Emit a single tick event with the latest play so the dashboard refreshes
  if (playsAdvanced > 0) {
    const freshGame = StateManager.get()?.schedule?.[gameIdx];
    const latestPlay = freshGame?.plays?.[freshGame.livePlayIndex];
    if (latestPlay) {
      EventBus.emit('game:tick', { play: latestPlay, game: freshGame });
    }
  }
...
function _initSettingsPickers() {
  ...
  document.getElementById('league-teams-reset-btn')?.addEventListener('click', () => {
    _renderLeagueTeamsList(_getLeagueTeamDefaults());
  });
...
const _LEAGUE_TEAM_DEFAULTS_BY_REGION = {
  north: [
    { city: 'Ironfield',   nickname: 'Bears',       abbr: 'IRF' },
    { city: 'Greystone',   nickname: 'Foxes',       abbr: 'GRS' },
    { city: 'Northhaven',  nickname: 'Pines',       abbr: 'NHV' },
    { city: 'Ridgecrest',  nickname: 'Owls',        abbr: 'RDC' },
    { city: 'Hollowbrook', nickname: 'Herons',      abbr: 'HLB' },
    { city: 'Coldwater',   nickname: 'Elk',         abbr: 'CLW' },
    { city: 'Frostburg',   nickname: 'Lynx',        abbr: 'FBG' },
    { city: 'Ashmont',     nickname: 'Ravens',      abbr: 'ASH' },
    { city: 'Clearfield',  nickname: 'Bucks',       abbr: 'CLF' },
  ],
  south: [
    { city: 'Bayside',     nickname: 'Pelicans',    abbr: 'BSY' },
    { city: 'Palmetto',    nickname: 'Kings',       abbr: 'PLM' },
    { city: 'Sunridge',    nickname: 'Hawks',       abbr: 'SRG' },
    { city: 'Copperfield', nickname: 'Bulls',       abbr: 'CPF' },
    { city: 'Bayou',       nickname: 'Herons',      abbr: 'BYO' },
    { city: 'Crestview',   nickname: 'Gators',      abbr: 'CRV' },
    { city: 'Suncoast',    nickname: 'Tarpon',      abbr: 'SCT' },
    { city: 'Delta',       nickname: 'Cranes',      abbr: 'DLT' },
    { city: 'Mossglen',    nickname: 'Foxes',       abbr: 'MSG' },
  ],
  east: [
    { city: 'Harborview',  nickname: 'Gulls',       abbr: 'HBR' },
    { city: 'Tideland',    nickname: 'Anchors',     abbr: 'TDL' },
    { city: 'Seagrove',    nickname: 'Osprey',      abbr: 'SVL' },
    { city: 'Lightshore',  nickname: 'Breakers',    abbr: 'LSH' },
    { city: 'Millbay',     nickname: 'Sails',       abbr: 'MBY' },
    { city: 'Cliffport',   nickname: 'Herons',      abbr: 'CLP' },
    { city: 'Cheswick',    nickname: 'Skippers',    abbr: 'CHP' },
    { city: 'Reedmont',    nickname: 'Cranes',      abbr: 'RMD' },
    { city: 'Gulfport',    nickname: 'Mullets',     abbr: 'GLP' },
  ],
  west: [
    { city: 'Mesaview',    nickname: 'Condors',     abbr: 'MSV' },
    { city: 'Peakstone',   nickname: 'Pumas',       abbr: 'PKS' },
    { city: 'Canyon',      nickname: 'Rams',        abbr: 'CYN' },
    { city: 'Dryrock',     nickname: 'Hawks',       abbr: 'DRT' },
    { city: 'Highgrass',   nickname: 'Coyotes',     abbr: 'HGS' },
    { city: 'Arroyo',      nickname: 'Dust Devils', abbr: 'ARD' },
    { city: 'Ridgeway',    nickname: 'Eagles',      abbr: 'RDW' },
    { city: 'Sundusted',   nickname: 'Vipers',      abbr: 'SDT' },
    { city: 'Saltflat',    nickname: 'Scorpions',   abbr: 'SLF' },
  ],
};

// Helper to get defaults for current save's region
function _getLeagueTeamDefaults() {
  const region = StateManager.get()?.settings?.region || 'north';
  return _LEAGUE_TEAM_DEFAULTS_BY_REGION[region] || _LEAGUE_TEAM_DEFAULTS_BY_REGION.north;
}
...
function _applyLeagueTeams() {
  ...
  StateManager.mutate(st => {
    (st.leagueTeams || []).forEach((t, i) => {
      ...
      // Cascade name change to user schedule opponent strings.
      // game.opponent is used by _activateGame to find the leagueTeam object —
      // if the name changed but schedule strings weren't updated, opponent lookup fails.
      if (oldName !== t.name && st.schedule) {
        for (const game of st.schedule) {
          if (game.opponent === oldName) game.opponent = t.name;
        }
      }

      // Cascade abbr change to league schedule game entries
